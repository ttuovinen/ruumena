...
sijan pehmeän siimes sois!`,
  },
  {
    ...
    content: `Kautta maan kuului helpotuksen huokaus, kun saatiin tietää että Keisarillinen Majesteetti viime tammikuun 21 p:nä on nähnyt hyväksi noudatettavaksi armossa vahvistaa eduskunnan hyväksymän esityksen siitä verosta, joka paloviinan valmistuksesta Suomessa oli suoritettava tänä kuluvana vuonna. Vastoin odotuksia ja niitä toimenpiteitä, jotka olivat kasaautumassa kansan tuiki tarpeellisen ja terveellisen menestymisen tielle oli tämänkin kysymyksen pikainen ja mutkaton ratkaiseminen valon välkähdyksenä valtiollisen elämämme synkän yön pimeydessä. Ainakin osa Suomen kansaa sai kiitollisuudella tuntea rohkaisua oikean asian lopullisesta voitosta. Toivolla loi se eteenpäin katseensa siitä, että niin tulee vastaisuudessakin käymään, kun ristiriitaisuuden usvia käy keskinäisen sopusoinnun kirkas päivä hajottamaan. Me olemme näinä kuluneina kuukausina eläneet sen veron alaisina, että valmistetusta paloviinasta on maksettava 2 markkaa kultakin litralta säännöllisväkevää paloviinaa. Mutta ehdotuksessa armolliseksi asetukseksi tästä asiasta tulevaa vuotta varten esitetään kahden markan vero aleunettavaksi l markan 40 pennin veroksi sillä olettamuksella, että nykyään voimassa oleva 2 markan vero voipi aiheuttaa halpojen väkijuomien maahan lisääntyvän tuonnin. Mutta kun ei ole vielä mitään todellista kokemusta siitä, voipi syystä panna tämän olettamuksen perään hyvinkin näkyvän kysymysmerkin. Ja vaikkapa vielä jotkut asiantuntevat viranomaiset, kuten varsin uskottavaa on, pitävät tämän vaaran todennäköisenä, niin voipihan tähän taas lisäksi vielä panna aivan suuren huutomerkin sekä samalla jäädä odottamaan nykyisen verotuksen todellisia eikä vain todennäköisiä seurauksia.

Vielä lisäksi uskon, että yhtä hyvin voipi osa Suomen kansaa ilman epäilyksiä kohdata näitä todennäköisiksi oletettuja seurauksia, pannen vakuutuksella sen päämaalikseen, että väkijuomamyrkyn valmistus ja kauppa maassamme, kuten jo on osaksi tapahtunut, supistuu kansan luottamus kirkollisiin kouluihin kasvaa, ja sillä niiden ohjelmassa tulee uskonto aina olemaan oppiaineista tärkeimpänä.`,
  },
];
